...
public class World_Mesh
{
    ...
    public World_Mesh(Transform parent, Vector3 localPosition, Vector3 localScale, float eulerZ, float meshWidth, float meshHeight, Material material, UVCoords uvCoords, int sortingOrderOffset)
    {
        ...
        Debug.Log($"World_Mesh created with position {localPosition}, scale {localScale}, and rotation {eulerZ}");
        ...
        Debug.Log($"Material assigned with mainTexture width: {material.mainTexture.width}, height: {material.mainTexture.height}");


        SetSortingOrderOffset(sortingOrderOffset);
        Debug.Log($"Sorting order set with offset {sortingOrderOffset}, total {GetSortingOrder()}");
    ...
    public void SetUVCoords(UVCoords uvCoords)
    {
        ...
        ApplyUVToUVArray(uvArray, ref uv);
        mesh.uv = uv;
        Debug.Log($"UVCoords set: {uvCoords.x}, {uvCoords.y}, {uvCoords.width}, {uvCoords.height}");
    }
